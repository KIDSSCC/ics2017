...
typedef struct {
    /*
     * DHelper在decode.h中，是一个函数指针，返回值类型为void，参数类型为vaddr_t，vaddr_t是uint32_t类型
     * EHelper在exec.h中，也是一个同样的函数指针
     */
  DHelper decode;
  EHelper execute;
  int width;
...
//下面的宏定义都是为了形成一个三元组，其中前两个函数为decode_XX,exec_XX
...
/*
 * 产生一个函数
 * static void exec_name(vaddt_t* eip)
 * {
 *  idex(......)
 * }
 */
...
void exec_wrapper(bool print_flag) {
  ...
  /*
   * exec_real是通过宏定义的方式实现的
   * 原始宏定义在exec.h中
   * 函数的声明在prefix.c中
   * 函数的实现在上面make_EHelper（real）
   */
  exec_real(&decoding.seq_eip);
  //printf("eip=%08x,and eax=%08x\n",cpu.eip,cpu.eax);
#ifdef DEBUG
